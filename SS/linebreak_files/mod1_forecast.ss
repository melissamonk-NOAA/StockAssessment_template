...
2 # MSY: 1= set to F(SPR); 2=calc F(MSY); 3=set to F(Btgt) or F0.1; 4=set t
# o F(endyr) 
...
#  endyr)
 0 0 0 0 -999 0
0 # Forecast selectivity (0=fcast selex is mean from year range; 1=fcast se
# lectivity from annual time-vary parms)
1 # Control rule method (1: ramp does catch=f(SSB), buffer on F; 2: ramp do
# es F=f(SSB), buffer on F; 3: ramp does catch=f(SSB), buffer on catch; 4: 
# ramp does F=f(SSB), buffer on catch) 
...
0 #_Forecast recruitment:  0= spawn_recr; 1=value*spawn_recr_fxn; 2=value*V
# irginRecr; 3=recent mean from yr range above (need to set phase to -1 in 
...
2021 # Rebuilder:  first year catch could have been set to zero (Ydecl)(-1 
# to set to 1999)
2021 # Rebuilder:  year for current age structure (Yinit) (-1 to set to end
# year+1)
